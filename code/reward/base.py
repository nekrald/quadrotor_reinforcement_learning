    ...
    def compute_reward(self, quad_state, quad_vel, collision_info):
        if collision_info.has_collided:
            ...
            for camera_id in self.used_cams:
                ...
                depth_perspective_array = np.array(
                        responses[0].image_data_float)
                depth_vis_array = np.array(
                        responses[1].image_data_float)
                depth_planner_array = np.array(
                ...
                min_depth_perspective = min(
                        results[0], min_depth_perspective)
...
class LandscapeReward(object):
    ...
    def compute_reward(self, quad_state,
            quad_prev_state, collision_info):
        state = np.array(list((
            quad_state.x_val, quad_state.y_val, 0)))
        prev_state = np.array(
                list((quad_prev_state.x_val,
                    quad_prev_state.y_val, 0)))
        if collision_info.has_collided:
            reward = self.collision_penalty
        else:
...
class CorridorReward(object):
    ...
    def compute_reward(self, quad_state, quad_vel, collision_info):
        if collision_info.has_collided:
            ...
            current_point_xy = np.array(
                    [quad_state.x_val, quad_state.y_val, 0])
            reward += cityblock(self.starting_point, current_point_xy)

            z_penalty = euclidean(
                    self.starting_point,
                    np.array([0, 0, quad_state.z_val]))
...
def make_reward(config, client):
    ...
    if reward_type == RewardType.EXPLORATION_REWARD:
        ...
        goal_point = np.array(
                reward_config[RewardConfigKeys.LANDSCAPE_GOAL_POINT])
        dist_penalty = reward_config[
                RewardConfigKeys.LANDSCAPE_LARGE_DIST_PENALTY]
        dist_coef = reward_config[
                RewardConfigKeys.LANDSCAPE_LARGE_DIST_COEF]
        reward = LandscapeReward(
                goal_point, collision_penalty,
                dist_penalty, dist_coef, client)
        ...
        height_penalty = reward_config[
                RewardConfigKeys.EXPLORE_HEIGHT_PENALTY]
        reward = CorridorReward(
                client, collision_penalty, height_penalty,
                used_cams, vehicle_rad, thresh_dist,
                goal_id, max_height)
    else:
